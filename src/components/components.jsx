        ...
        <nav style={{ display: "flex", alignItems: "center", gap: "0.25rem" }} className="hide-md">
          <div ref={ref} style={{ position: "relative" }}>
            ...
            <div className="dropdown">
                {productLinks.map((l) =>
              <a key={l.to} href={l.to} onClick={() => setProductOpen(false)}>{l.label}</a>
              )}
              </div>
            }
          </div>
          <div ref={ucRef} style={{ position: "relative" }}>
            ...
            <div className="dropdown">
                {useCasesLinks.map((l) =>
              <a key={l.to} href={l.to} onClick={() => setUseCasesOpen(false)}>{l.label}</a>
              )}
              </div>
            }
          </div>
...
export function FooterCol({ title, links, items }) {
  return (
    <div>
      ...
      <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: ".55rem" }}>
        ...
        links.map(([label, to]) =>
        <li key={label}>
            {to.startsWith("/") ? <a href={to} style={{ color: "rgba(255,255,255,0.6)", fontSize: ".9rem" }}>{label}</a> :
